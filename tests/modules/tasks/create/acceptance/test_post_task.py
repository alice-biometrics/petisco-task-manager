...
def test_should_return_a_405_when_post_task_with_no_data(
    ...
    assert response.json == {'error': {'message': 'Missing Input Title', 'type': 'MissingInputHttpError'}}


@pytest.mark.acceptance
def test_should_return_a_405_when_post_task_with_no_title(
    client, given_any_description
):
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
...
@pytest.mark.acceptance
def test_should_return_a_405_when_post_task_with_no_description(
