...
from taskmanager.src.modules.tasks.application.retrieve.retrieve_task import RetrieveTask
...
def test_should_return_task_not_found_error(given_any_task_id, given_any_task):
    ...
    mock_task_repository = Mock(spec=ITaskRepository)
    mock_task_repository.retrieve = Mock(return_value=Failure(TaskNotFoundError(given_any_task_id)))
