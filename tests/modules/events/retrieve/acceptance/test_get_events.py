...
def test_should_return_a_200_when_get_events(
    ...
    response = petisco_client.get("/taskmanager/events", headers=headers)
    assert response.status_code == 200


@pytest.mark.acceptance
def test_should_return_a_200_when_get_events_of_non_existent_task(petisco_client):

    response = petisco_client.get("/taskmanager/events")
